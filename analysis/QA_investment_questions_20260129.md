...
|---------|--------|-------------|
| Cohesion | 군집성 | Theme internal correlation strength (Fiedler eigenvalue) |
| Breakout | 모멘텀 | Momentum stage (Transition + Above BB) |
| Signal | 시그널 | Trading signal from meta-labeling model |
| Key Player | 핵심 종목 | High PageRank centrality within theme |
| TIER | 티어 | Theme quality classification |
...
**Answer**: 모멘텀 종목 (Transition regime + Above BB)

| Rank | Ticker | Score | Dev_BB_Pct | Key Themes |
|------|--------|-------|------------|------------|
...
**Answer**: Top 10 by PageRank centrality in TIER 1 themes

| Ticker | PageRank | Theme | Role |
|--------|----------|-------|------|
...
**Answer**: 통신 theme PageRank ranking

| Rank | Ticker | PageRank | Notes |
|------|--------|----------|-------|
...
**Answer**: 은행 theme PageRank ranking
...
**Answer**: Fiedler eigenvalue 기준 순위 (높을수록 군집성 강함)

| Rank | Theme | Fiedler | Interpretation |
|------|-------|---------|----------------|
...
**군집성 해석 가이드**:
- Fiedler > 3.0: 매우 강함 (테마 내 높은 상관관계)
- Fiedler 1.0-3.0: 강함
- Fiedler 0.5-1.0: 보통
- Fiedler < 0.5: 약함 (분산된 테마)
...
| Theme | Fiedler | Confidence | Recommendation |
|-------|---------|------------|----------------|
...
**Note**: HMM regime detection was NOT run this session. Use the linked report for current regime state.
...
### Q11: How far above Bollinger Band are momentum stocks? (모멘텀 종목의 BB 이탈 정도는?)
**Answer**: Dev_BB_Pct가 모멘텀 강도를 나타냄

| 리스크 수준 | Dev_BB_Pct | 종목 |
|-------------|------------|------|
...
**해석**: Dev_BB_Pct가 높을수록 모멘텀은 강하지만 과열 가능성도 있음

---

### Q12: Which themes have weak cohesion? (군집성이 약한 테마는?)
**Answer**: Fiedler < 1.0인 테마
...
### Q14: How does Transition regime perform historically?
**Answer**: Transition regime performance
